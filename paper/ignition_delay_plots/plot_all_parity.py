...
# "As mentioned above, both argon and nitrogen were used in the RCM experiments,
# while in the shock-tube experiments only nitrogen was used as the diluent gas. "
...
tables_to_plot = [15, 16, 17, 18, 19, 20]
...
for table_index in tables_to_plot:
    ...
    if P[0] == 0:
        P = one_table['partial pressure / bar'].values * ct.one_atm  # pressures in atm
    ...
    rmg_points += list(base_rmg_delays)
    improved_points += list(improved_rmg_delays)
    experimental_points += list(tau / 1000.0)


# make a parity plot
plt.figure(figsize=(8, 8), dpi=80)
plt.scatter(experimental_points, rmg_points)
plt.scatter(experimental_points, improved_points)

# plot a diagonal line
xmin = 0.9 * np.minimum(np.min(experimental_points), np.min(experimental_points))
xmax = 1.1 * np.maximum(np.max(experimental_points), np.max(experimental_points))
diagonal = np.linspace(xmin, xmax, 101)
plt.plot(diagonal, diagonal, color='black')

plt.xscale("log")
plt.yscale("log")
plt.xlabel('experimental delay')
plt.ylabel('delay')
plt.legend(['Base RMG', 'Improved RMG', 'Parity'])
plt.savefig(os.path.join(this_dir, f'parity.png'))
...
# plt.title('Ignition Delays $\phi$=' + f'{phi}, P={np.average(P)}')
# plt.xlabel('1000K / T')
# plt.ylabel('Delay (s)')
